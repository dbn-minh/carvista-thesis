...
export default function ListingPreviewCard({ form, selectedVariant }: Props) {
  ...
  return (
    <article className="overflow-hidden rounded-[30px] border border-cars-gray-light/70 bg-white shadow-[0_18px_40px_rgba(15,45,98,0.08)]">
      <div className="relative h-56 overflow-hidden bg-[linear-gradient(135deg,rgba(233,241,255,0.9),rgba(255,255,255,1))]">
        {coverPhoto && safeCoverSrc ? (
          <img src={safeCoverSrc} alt={coverPhoto.name} className="h-full w-full object-cover" />
        ) : (
          <div className="flex h-full items-center justify-center px-8 text-center text-sm font-medium text-cars-gray">
      ...
      <div className="space-y-4 p-5">
        ...
        <div className="flex flex-wrap gap-2">
          {form.mileageKm ? (
            <span className="rounded-full bg-cars-off-white px-3 py-1.5 text-xs font-semibold text-cars-primary">
              {form.mileageKm} km
            </span>
          ) : null}
          {form.condition ? (
